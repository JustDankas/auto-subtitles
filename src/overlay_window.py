...
BOX_SPACING = 6  # vertical gap between boxes
PANEL_MARGIN = 16
...
class SubtitleOverlay(QWidget):
    def __init__(
        ...
        self._layout.addStretch()  # keeps boxes bottom-aligned; overflow clips above the window
    ...
    def update_partial(self, text: str) -> None:
        ...
        if self._partial_widget is None:
            self._partial_widget = SubtitleLineWidget(
                text, self._new_text_color, self._box_width, self._font
            )
            self._layout.addWidget(self._partial_widget)
        else:
            self._partial_widget.set_text(text)
